-- ============================================================================
-- MINEGUARD LIBERIA — Phase 06 fix (3): add the missing `sites` audit trigger
--
-- Gap found during Phase 06 cascade verification: the Phase 06 audit trigger
-- set (migration 20260903000050) covered organization_members, site_members,
-- organizational_units, workers, org_invites and organizations — but NOT
-- `sites`. Sites are a first-class tenant-management table (site create /
-- update / delete / county / status changes); the RLS_MATRIX / SECURITY_MODEL
-- audit requirements ("site setting changes") require them to be audited too.
--
-- This migration:
--   1. Re-creates trg_audit_capture() with a `sites` branch (same actor /
--      append-only / org-scope-nulling semantics as the other tables, keeping
--      the generalized fix-52 behavior: organization_id is nulled whenever the
--      referenced org no longer exists, e.g. cascade deletes).
--   2. Creates trg_audit_sites on public.sites.
-- ============================================================================
begin;
...
comment on function public.trg_audit_capture() is
    'Phase 06: server-side audit capture for tenant-management tables (incl. sites). Actor = auth.uid() (null under service-role writes). Append-only: inserts into audit_log only; never raises. organization_id is nulled when the referenced org no longer exists (org deletion / cascade), preserving the record with platform scope.';
