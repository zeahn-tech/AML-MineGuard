...
| Field | Value |
|---|---|
| Doc status | **IMPLEMENTED (Phases 03–04 + 06–09 + 11)** — catalogs + helpers live in `20260903000030_phase03_rbac.sql`; site-scoped resolution landed with Phase 04; Phase 06 RLS enforcement over tenant-management tables; safety-domain policies live via Phases 07–09; **Phase 11 (2026-09-09)**: government role family now has live membership flows — regulator users hold government role codes in `organization_members` (check constraint extended per fix …092), `bootstrap_first_regulator_admin` claims the first open regulator org, `regulator_update_user_role` assigns the 4 seeded government codes, and government permissions (`workers.view`/`users.view`/`audit_logs.view`) are evaluated at the regulator org inside grant-gated policies (verified by `verify-phase11.mjs` 48/48 PASS). Still open: platform membership flows + per-org custom roles (Phase 12). |
