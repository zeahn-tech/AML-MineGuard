...
| Field | Value |
|---|---|
| Doc status | BASELINE from Phase 00 audit + TARGET model below. **Phase 11 landed 2026-09-09**: government authorization model live — `government_grants` (the explicit record behind every cross-org regulator read; one active grant per regulator org/user/target org/site; expiry supported; revocation immediate), grant-gated regulator SELECT policies on 16 tables (no grant ⇒ zero rows), regulator bootstrap/grant RPCs with server-side authorization, `government_grants` audit trigger (20 total) — live-verified on `vuniwebbrvpgxscdsfei` (`verify-phase11.mjs` 48/48 PASS; Phase 06–10 regression green). Phase 09 landed 2026-09-04: emergency/SOS RLS + lifecycle guards (19 triggers). Phases 06–08 landed for tenant-management + safety-domain. Remaining safety-domain tables (notices, documents) ungated until Phase 10+. |
