...
| Field | Value |
|---|---|
| Doc status | **Phase 11 LANDED + LIVE-VERIFIED (2026-09-09, project `vuniwebbrvpgxscdsfei`)** — government grant-gated regulator SELECT (16 policies: incidents/evidence/witnesses/jsas/inspections/CAPA/emergency×5/sites/units/workers/org-members/audit_log; every policy requires an active `government_grants` row covering (org, site); permission checks evaluated at the REGULATOR org per fix …091; verify-phase11 48/48 PASS). Phase 09 LANDED for emergency response + SOS. Phase 08 LANDED for JSA + inspection + CAPA. Phase 07 LANDED for incident/evidence/witnesses. Phase 06 LANDED for tenant-management + audit. Enforcement backend (ADR-008): **Supabase PostgreSQL RLS**. Remaining: §1.5 notices, §1.7 documents (Phase 10+). |
