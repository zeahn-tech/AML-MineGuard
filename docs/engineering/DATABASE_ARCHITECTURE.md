...
| Field | Value |
|---|---|
| Doc status | BASELINE + TARGET (decided 2026-09-03: target database = Supabase PostgreSQL, ADR-008). Phases 01–04 schema live. Phase 06 live: RLS enforcement + `audit_log` + audit triggers on 7 tenant-management tables + sites (migrations `…050`–`…053`). Phase 07 live: incidents/evidence/witnesses + storage bucket + 3 safety-domain audit triggers (10 total) — `…060`/`…061`. Phase 08 live: jsas/jsa_steps + inspections + corrective_actions + 2 audit triggers (14 total) + fix migrations `…072`/`…073`. Phase 09 live: emergency/SOS tables + 5 audit triggers (19 total) + fixes `…081`–`…084` — live-verified (`verify-phase09.mjs` all-PASS). Phase 11 live: `government_grants` (explicit regulator authorization; partial unique active-grant index) + 4 helpers + 4 RPCs + 16 grant-gated regulator SELECT policies + `government_grants` audit trigger (20 total) + fixes `…091`–`…093` — live-verified (`verify-phase11.mjs` 48/48 PASS). Phase 12 live: `plans` (seeded tiers) + `subscriptions` (partial unique one-active-per-org index) + site/settings/subscription/platform/grant-expiry RPCs (`…094`) + server-side `gov_national_overview()` + dedicated `trg_audit_subscriptions` trigger (21 total) + probe-driven fix `…095` (grant-issue result capture, audit_log column names, platform-list explicit gate, legacy 4-arg grant overload dropped for PostgREST resolution) — live-verified (`verify-phase12.mjs` 31/31 PASS self-cleaning). Session 18 live (`…099_org_lifecycle.sql`): `create_organization(p_name, p_org_type, p_county)` SECURITY DEFINER (atomic org + owner membership + starter subscription; server-generated collision-safe slug `name`, `name-2`, …; UUID remains the PK; created_by/ownership derived from `auth.uid()`; regulator/platform types rejected; audit via the existing organizations + organization_members triggers) + `org_transfer_ownership(p_organization_id, p_new_owner_user_id)` (current-owner-only atomic swap: target→owner, previous owner→admin; exactly one active owner invariant) + `organizations_org_type_check` extended with `service_provider` — live-verified (`verify-org-lifecycle.mjs` 30/30 PASS self-cleaning). |
